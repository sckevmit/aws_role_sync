...
smc = session.client('secretsmanager')
...
def init_function():
    try:
        rsp = smc.get_secret_value(SecretId=secret_id)
        ...
        secret = json.loads(rsp.get('SecretString'))
        azure_user = secret.get('AzureUser')
        azure_pass = secret.get('AzurePassword')
        azure_tenant = secret.get('AzureTenantName')
        azure_tenant_id = secret.get('AzureTenantId')
        sso_app_id = ssmc.get_parameter(Name=f'{PREFIX}.appId', WithDecryption=True)['Parameter']['Value']
        msiam_access_id = ssmc.get_parameter(Name=f'{PREFIX}.msiam_access_id', WithDecryption=True)
